...
import { Card, Row, Col, Button, message, Radio, Space } from 'antd';
...
const SyllabusShows = () => {
  ...
  useEffect(() => {
    React.$axios.post('/findSyllabusById', { _id: id }).then((syllabusData) => {
      setSyllabusState(syllabusData.data[0].status)
  ...
  const handleSubmit = async () => {
    ...
    console.log(params)
    const res = await React.$axios.post('/updateSyllabusState', params)
    if (res && res.isSucceed) {
      message.success('审批成功')
      history.replace('/progress/syllabus')
    }
  }
